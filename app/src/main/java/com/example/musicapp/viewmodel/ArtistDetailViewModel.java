...
public class ArtistDetailViewModel extends AndroidViewModel {
  private final MutableLiveData<Artist> _artistDetail = new MutableLiveData<>();
  private final MutableLiveData<List<AlbumSimplified>> _artistAlbums = new MutableLiveData<>();
  private final MutableLiveData<List<Song>> _artistTopSongs = new MutableLiveData<>();
    ...
    db.collection("users")
                ...
                userDoc
                    ...
                    .addOnSuccessListener(
                        aVoid -> {
                          Toast.makeText(
                                  context,
                                  "Removed from followed artists successfully",
                                  Toast.LENGTH_SHORT)
                              .show();
                          //                                    followedArtists.removeIf(artist ->
                          // artist.getId().equals(artistId));
                        })
                    .addOnFailureListener(
                        e ->
                            Log.e(
                                "FollowedArtistAdapter",
                                "Failed to remove artist from followed artists: "
                                    + e.getMessage()));
              } else {
        ...
        .addOnFailureListener(
            e ->
                Log.e(
                    "FollowedArtistAdapter",
                    "Failed to retrieve user document: " + e.getMessage()));
  }
                ...
                userDoc
                    ...
                    .addOnSuccessListener(
                        aVoid ->
                            Toast.makeText(
                                    context,
                                    "Added to followed artists successfully",
                                    Toast.LENGTH_SHORT)
                                .show())
                    .addOnFailureListener(
                        e ->
                            Log.e(
                                "FollowedArtistAdapter",
                                "Failed to add artist to followed artists: " + e.getMessage()));
              } else {
                Log.e("FollowedArtistAdapter", "No user document found with userId: " + userId);
              }
            })
