...
public class PlaylistDetailViewModel extends AndroidViewModel {
          ...
          new Callback<SimplifiedTrack>() {
            ...
            public void onResponse(Call<SimplifiedTrack> call, Response<SimplifiedTrack> response) {
              if (response.isSuccessful()) {
                ...
                Log.e(
                    "PlaylistDetailViewModel", "Error fetching Spotify track: " + response.code());
              }
            }
  ...
  public void updatePlaylist(Playlist playlist, String newName, Uri selectedImageUri) {
    if (selectedImageUri != null && !selectedImageUri.toString().equals(playlist.getImageURL())) {
      StorageReference storageRef =
          storage.getReference().child("playlist/" + UUID.randomUUID().toString());
      storageRef
          ...
          .addOnFailureListener(
              e -> {
                Toast.makeText(getApplication(), "Failed to upload image", Toast.LENGTH_SHORT)
                    .show();
  ...
  private void updatePlaylistInFirestore(Playlist playlist, String newName, String newImageURL) {
    db.collection("playlists")
        ...
        .addOnSuccessListener(
            aVoid -> {
              Toast.makeText(getApplication(), "Playlist updated successfully", Toast.LENGTH_SHORT)
                  .show();
        ...
        .addOnFailureListener(
            e -> {
              Toast.makeText(getApplication(), "Failed to update playlist", Toast.LENGTH_SHORT)
                  .show();
  ...
  public void deletePlaylist(Playlist playlist) {
    db.collection("playlists")
        ...
        .addOnSuccessListener(
            aVoid -> {
              Toast.makeText(getApplication(), "Playlist deleted successfully", Toast.LENGTH_SHORT)
                  .show();
            })
        .addOnFailureListener(
            e -> {
              Toast.makeText(getApplication(), "Failed to delete playlist", Toast.LENGTH_SHORT)
                  .show();
  ...
  //        Call<SimplifiedTrack> getTrack(@Header("Authorization") String authorization,
